...
from utils.file_manager import load_history, append_to_history
...
def fetch_article_content(url, session, content_selector, test):
    ...
    try:
        ...
        print(f"    ❌ 抓取正文失败: {e}")
        return ""
def fetch_incremental_notices(site_name, site_config, days_to_crawl, history_set, history_file, test):
    """
    ...
    for page in range(1, max_pages + 1):
        ...
        try:
            ...
            for item in items:
                ...
                print(f"  ✅ 发现新通知: {str(pub_date)} | [{dept}] {title}")
                print(f"    -> 准备抓取正文...")
                ...
                if article_content:
                    ...
                    # 成功抓取后，立刻写入本地日志并更新内存中的 history_set
